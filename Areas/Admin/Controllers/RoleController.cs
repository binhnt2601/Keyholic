...
using BanPhimCanhCach.Areas.Identity.Models.ManageViewModels;
using BanPhimCanhCach.Areas.Identity.Models.RoleViewModels;
using BanPhimCanhCach.Data;
using BanPhimCanhCach.ExtendMethods;
...
namespace BanPhimCanhCach.Areas.Admin.Controllers
{
    ...
    public class RoleController : Controller
    {
        ...
        [NonAction]
        [AllowAnonymous]
        public async Task GenerateAdminAccount()
        {
            var existedAdmin = _userManager.Users.FirstOrDefault(u => u.UserName == "admin123");
            if (existedAdmin != null)
            {
                _logger.LogError("Da ton tai tk Admin");
            }
            else
            {
                ...
                if (result.Succeeded)
                {
                    var resultAddRole = await _userManager.AddToRoleAsync(admin, Roles.Administrator);
                    _logger.LogInformation("Tao Admin thanh cong. \n TK: admin\n MK: Admin@123");
                }
                else
                {
                    _logger.LogInformation("Loi");
                }
            }
